...
# PATH to files with forecast data
PATH_GFS = "/home/kirill/Downloads/Data/gfs/"
PATH_COSMO = "/home/kirill/Downloads/Data/cosmo_true/"


def calc_point_number_gfs(lat, lon):
...
def calc_point_number_cosmo(lat, lon):
    """Calculation of the point number based on the specified coordinates."""
    lat_point = ((lat - 45) / 0.125 + 1)
    lon_point = ((lon - 53) / 0.125 + 1)
    lat_point = int(lat_point + (0.5 if lat_point > 0 else -0.5))
    lon_point = int(lon_point + (0.5 if lon_point > 0 else -0.5))
    point_number_in_gfc = 537 * (lat_point - 1) + lon_point
    return point_number_in_gfc


def calc_five_points(lat, lon):
    """The required point is taken and 4 points around."""
    need_point = calc_point_number_gfs(lat, lon)
    first_point = calc_point_number_gfs(lat - 0.5, lon - 0.5)
    second_point = calc_point_number_gfs(lat - 0.5, lon)
    third_point = calc_point_number_gfs(lat + 0.5, lon - 0.5)
    fourth_point = calc_point_number_gfs(lat + 0.5, lon + 0.5)
...
def add_data_to_the_speed_list(file, num_model):
    """Adding 1.file name, 2.year, 3.month, 4.day, 5.time."""
    if num_model == '1':
        wind_data = [file, file[slice(0, 4)], file[slice(4, 6)], int(file[slice(6, 8)]), int(file[slice(8, 10)])]
    elif num_model == '2':
        wind_data = [file, file[slice(0, 4)], file[slice(4, 6)], int(file[slice(6, 8)]), int(file[slice(11, 13)])]
    return wind_data


def calc_wind_speed(correct_data_VGRD, correct_data_UGRD, file, num_model):
    """Calculation of wind speed from vertical(VGRD) and horizontal(UGRD) wind components."""
    speed = add_data_to_the_speed_list(file, num_model)
...
def get_wind_speed_for_point(path, dirs, points_to_calculate, speed_wind, reference_num, num_model, low_limit, top_limit):
    ...
    for file, progress_bar in zip(sorted(os.listdir(path + dirs)), tqdm(sorted(os.listdir(path + dirs)))):
        if re.match('\d{10}_\d{2}', file):
            ...
            file_reader = pd.read_csv(current_file, sep='/s', skiprows=45, skipfooter=8, header=None, engine='python')
            ...
            for i in range(0, len(data_from_file)):
                ...
                if ((data_from_file[i][0] == "04") or (data_from_file[i][0] == "05")) and data_from_file[i][1] == '21':
                    if data_from_file[i][0] == '04':
                        correct_data_UGRD = []
                        correct_data_UGRD.append([data_from_file[i][0], data_from_file[i][1], data_from_file[i][points_to_calculate]])
                    if data_from_file[i][0] == '05':
                        correct_data_VGRD = []
                        correct_data_VGRD.append([data_from_file[i][0], data_from_file[i][1], data_from_file[i][points_to_calculate]])

            speed_wind.append(calc_wind_speed(correct_data_VGRD, correct_data_UGRD, file, num_model))
...
def get_wind_speed_for_five_point(dirs, first_point, second_point, third_point, fourth_point, need_point,
                                  speed_wind, reference_num, num_model, low_limit, top_limit):
    print(10*' ' + f'Directory {dirs} being processed')
    for file, progress_bar in zip(sorted(os.listdir(PATH_GFS + dirs)), tqdm(sorted(os.listdir(PATH_GFS + dirs)))):
        if re.match('\d{10}_\d{2}', file):

            current_file = PATH_GFS + dirs + '/' + file
            file_reader = pd.read_csv(current_file, sep='/s', skiprows=1, header=None, engine='python')
            data_from_file = file_reader.values.tolist()
            num_rows = 0
            for i in range(low_limit, top_limit):
                data_from_file[i] = data_from_file[i][0].split()
...
def main(value, choice_model):
    ...
    if choice_model == '1':
        ...
        points_to_calculate = calc_point_number_cosmo(lat, lon) + 1
        print(f"Point number  in gfs: {points_to_calculate - 1}")
        for dirs in sorted(os.listdir(PATH_COSMO)):
            if re.match('201[6,7]', dirs):
                speed_wind_training, reference_num = get_wind_speed_for_point(PATH_COSMO, dirs, points_to_calculate,
                                                                              speed_wind_training,
                                                                              reference_num, choice_model, 44, 45)
            elif re.match('2018', dirs):
                reference_num = 0
                speed_wind_test, reference_num = get_wind_speed_for_point(PATH_COSMO, dirs, points_to_calculate,
                                                                          speed_wind_test,
                                                                          reference_num, choice_model, 44, 45)

        write.write_in_file('/home/kirill/Downloads/Data/cosmo_data/', 'forecast_for_point_data_training_cosmo', speed_wind_training)
        write.write_in_file('/home/kirill/Downloads/Data/cosmo_data/', 'forecast_for_point_data_test_cosmo', speed_wind_test)

    elif choice_model == '2':
        lat = float(input('Input latitude: ') or 55.03)  # 55.03 - lat Tolmachevo
        lon = float(input('Input longitude: ') or 82.6)  # 82.6 - lon Tolmachevo
        if value == '1':
            points_to_calculate = calc_point_number_gfs(lat, lon) + 1
            print(f"Point number  in gfs: {points_to_calculate - 1}")
            for dirs in sorted(os.listdir(PATH_GFS)):
                if re.match('201[6,7]', dirs):
                    speed_wind_training, reference_num = get_wind_speed_for_point(PATH_GFS, dirs, points_to_calculate,
                                                                                  speed_wind_training,
                                                                                  reference_num, choice_model, 32, 40)
                elif re.match('2018', dirs):
                    reference_num = 0
                    speed_wind_test, reference_num = get_wind_speed_for_point(PATH_GFS, dirs, points_to_calculate,
                                                                              speed_wind_test,
                                                                              reference_num, choice_model, 32, 40)

            write.write_in_file(PATH_GFS, 'forecast_for_point_data_training', speed_wind_training)
            write.write_in_file(PATH_GFS, 'forecast_for_point_data_test', speed_wind_test)
            ...
            for dirs in sorted(os.listdir(PATH_GFS)):
                if re.match('201[6,7]', dirs):
                    ...
                    speed_wind_test, reference_num = get_wind_speed_for_five_point(dirs, first_point, second_point,
                                                                                   ...
                                                                                   speed_wind_test,
                                                                                   reference_num, choice_model, 32, 40)
            write.write_in_file(PATH_GFS, 'forecast_for_five_point_data_training', speed_wind_training)
            write.write_in_file(PATH_GFS, 'forecast_for_five_point_data_test', speed_wind_test)
