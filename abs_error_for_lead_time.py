...
def calc_abs_diff(lead_time, diff_num_lead_time, speed_wind_predictive_true, speed_wind_practical, i, j):
            ...
            if speed_wind_predictive_true[i][j][3] == int(speed_wind_practical[i + j * 8][3]):
                if float(speed_wind_predictive_true[i][j][5]) >= 9999 or math.isnan(float(speed_wind_practical[i + j * 8][5])):
                    ...
                    diff_num_lead_time.append(
                        math.fabs(float(speed_wind_practical[i + j * 8][5]) - float(speed_wind_predictive_true[i][j][5])))

    return diff_num_lead_time


def calc_abs(diff_lead_time):
    average_diff: int = 0
