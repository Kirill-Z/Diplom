            ...
            for i in range(0, 70):
                ...
                if (data_from_file[i][0] == "04" or "05") and data_from_file[i][1] == '21':
                    if data_from_file[i][0] == '04':
                        correct_data_UGRD = calc_point(getting_need_data_for_point(points_to_calculate, num_rows), num_rows)
                    if data_from_file[i][0] == '05':
                        correct_data_VGRD = calc_point(getting_need_data_for_point(points_to_calculate, num_rows), num_rows)
    ...
    for file in sorted(os.listdir(PATH)):
        if re.match('\d{10}_\d{2}', file):
            ...
            for i in range(0, 70):
                ...
                if (data_from_file[i][0] == "04" or "05") and data_from_file[i][1] == '21':
                    if data_from_file[i][0] == '04':
                        correct_data_UGRD = calc_for_area(getting_need_data_for_area(low_point_number, top_point_number, num_rows))
                    if data_from_file[i][0] == '05':
                        correct_data_VGRD = calc_for_area(getting_need_data_for_area(low_point_number, top_point_number, num_rows))
